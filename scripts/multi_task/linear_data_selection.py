...
class MultiTaskLinearWithDataSelection(MultiTaskLinear):
    ...
    def fit_new_task(self, X: np.ndarray, y: np.ndarray) -> dict:
        ...
        # Compute ω_n using the current prior
        w_n, _ = self._fit_model(X, y_internal, self.regularization, prior=prior)
        ...
        # Update prior using selected ω_s and ω_n
        if selected_weights:
            selected_weights.append(w_n)  # Include ω_n itself
        ...
        w_n, loss = self._fit_model(X, y_internal, self.regularization, prior=prior)
